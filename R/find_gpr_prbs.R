...
find_ale_frqs <- function(ind_gts) {
  ...
  dimnames(ale_frqs) = list(allele_code = paste0("A", 0:1), locus = colnames(ind_gts))
  ale_frqs
}
...
find_pss_frst_gt_prbs = function(pss_gt_prbs, L) {
  ...
  array(
    data = pss_gt_prbs[, rep(1:L, each = n.pss.gts)],
...
FindPssGpPsPOPs = function(ale_frqs, L, pss.gt.1.prbs) {
